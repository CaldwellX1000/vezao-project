...
export default function SearchPage() {
  ...
  const renderUserRow = (user: Profile, showFollow = false) => (
    ...
    >
      <div className="w-12 h-12 rounded-full bg-zinc-800 overflow-hidden shrink-0 ring-1 ring-white/10">
        {user.avatar_url ? (
  ...
  return (
    <div className="h-[100dvh] bg-black text-white flex flex-col overflow-hidden">
      <div className="shrink-0 z-50 bg-black/95 backdrop-blur-md border-b border-white/10 px-4 pt-3 pb-0">
        <div className="flex items-center gap-3 mb-3">
          <button onClick={() => router.back()} className="text-white/70 text-lg font-bold shrink-0">
            ←
          </button>
          <div className="flex-1">
            <input
              ...
              autoFocus
              className="w-full bg-zinc-900 border border-white/10 rounded-full px-4 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-pink-400/60"
            />
          </div>
        </div>
          ...
          {(['users', 'videos', 'hashtags'] as Tab[]).map((t) => (
            <button
              ...
              className={`flex-1 py-2.5 text-sm font-semibold relative ${
                tab === t ? 'text-pink-400' : 'text-white/40'
              }`}
            >
              {t === 'users' ? 'Users' : t === 'videos' ? 'Videos' : 'Hashtags'}
              {tab === t && (
                <span className="absolute bottom-0 left-1/2 -translate-x-1/2 w-10 h-0.5 bg-pink-400 rounded-full" />
              )}
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 overflow-y-auto overscroll-y-contain px-4 pt-4 pb-24 scrollbar-hide">
        {loading ? (
          ...
          tab === 'users' && suggested.length > 0 ? (
            <div>
              <p className="text-sm font-semibold text-white/50 mb-2 px-2">
                Suggested accounts
              </p>
